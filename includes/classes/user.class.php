...
class User {
	...
	protected static $db_fields = array('id', 'user_id', 'first_name', 'last_name', 'gender', 'username', 'password', 'email', 'user_level', 'primary_group', 'activated', 'suspended', 'date_created', 'last_login', 'account_lock', 'signup_ip', 'last_ip', 'country', 'whitelist', 'ip_whitelist', 'tokens', 'bank_tokens', 'invited_by', 'created', 'expires', 'expiry_date', 'level_id', 'level_name', 'auto', 'datetime', 'ip_address', 'name', 'qty', 'status', 'redirect_page', 'access_time', 'time_type', 'amount', 'created', 'timed_access', 'expiry_date', 'expires', 'package_name','salt','oauth_provider','oauth_uid','price','failed_logins','last_failed','description','count','login_count','f_count','tadmin','ime2','tvrtka','naziv');
	    ...
	    $datetime = strftime("%Y-%m-%d %H:%M:%S", time());
   		...
   		if($allowed === true){
			...
			if($failed === false){
   			...
   			$session->message('<center><hr style="color:red;width:98%"><h3 style="font-size:17px">
			You have to mutch incorrect login try, you cannot use system for while.
			<center/></h3><hr style="color:red;width:98%"></center><br>');
	...
	$sql = "SELECT * FROM  ".self::$table_name." WHERE username = '{$username}' AND password = '{$password2}' LIMIT 1";
    $result_array = self::find_by_sql($sql);
		return !empty($result_array) ? true : false;
	}
	...
	public static function check_login($username, $password2, $current_ip, $remember) {
		// instantiate Session Class
		$session = new Session();
		
	    // Check database to see if username/password exist.
		$found_user = self::authenticate($username, $password2);
			   	...
			   	if(!$suspended){
					if($whitelist) {
						...
						if (in_array($current_ip, $whitelist)) {
							...
							// echo "failure";
							$session->message("<div class='alert alert-warning'><button type='button' class='close' data-dismiss='alert'>×</button>
							For this user account whitelist was active and your current IP addresa:($current_ip) was not allowed.</div>");
							return "false";
						}
					} else {
						$session->login($found_user, $remember);
					...
					$session->message("<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Your account was suspended, please contact your administor.</div>");
					return "false";
				 }
			 } else {
				$session->message("<div class='alert alert-info'><button type='button' class='close' data-dismiss='alert'>×</button>Your account was not yet confirmed, please check your email for activation link. 
				For new activation link <a href='activate.php'>press here.</a></div>");
				return "false";
			 }
		} else {
			if(!isset($_SESSION['message'])){
				$session->message("Username or password was incorrect.");
			}
			return "false";
		}
   }
	...
	public function create_account($username, $password, $email, $first_name, $last_name, $signup_ip, $country, $gender, $invite_code=""){
		global $database;
		$session = new Session();
		...
		if ($flag == true){
			...
			$sql = "INSERT INTO ".self::$table_name." VALUES ('', '$user_id', '$first_name', '$last_name', '$gender', '$username', '$password', '$email', '$user_level', '$user_level', '$activated', '0', '$datetime', '', '0', '$signup_ip', '', '$country', '0', '', '', '', '$invited_by','$salt','','','0')";
			$database->query($sql);
			...
			$session->message("<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'>×</button>Ova opcija je privrenemo nedostupna.</div>");
	...
	public static function purchase_access($user_id, $id){
		...
		if($user->tokens < $package->amount){
			$session->message("<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Pogreška 13.</div>");
		} else {
			$current_access = explode(",", $user->user_level);
			if(in_array($id, $current_access)){
				$session->message("<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Pogreška 14.</div>");
			} else {
				...
				$session->message("<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'>×</button>Pogreška 15.</div>");
			}
		}
	...
	public static function add_access($user_id, $id){
		...
		$database->query("INSERT INTO levels VALUES('','{$user_id}','{$id}','{$datetime}','{$package->timed_access}','{$expiry_date}')  ");
		$session->message("<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'>×</button></div>");
		// redirect_to("dashboard.php?page=settings");
	}
	...
	public static function delete_token_package($id){
		global $database;
		$session = new Session;
		$database->query("DELETE FROM token_packages WHERE id = '{$id}' ");
	...
	public static function transfer_tokens($user_id, $username, $amount){
		...
		if($amount > User::get_current_tokens($user_id)){
			$session->message("<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Sorry, but you don't have enough tokens to do that.</div>");
		} else {
