...
class Admin {
	...
	protected static $db_fields = array('vpnuserreg', 'id', 'user_id', 'first_name', 'last_name', 'gender', 'username', 'password', 'email', 'user_level', 'activated', 'suspended', 'date_created', 'last_login', 'account_lock', 'signup_ip', 'last_ip', 'country', 'whitelist', 'ip_whitelist', 'tokens', 'bank_tokens', 'primary_group', 'created', 'expires', 'expiry_date', 'level_id', 'id', 'message', 'date','count','f_count','t_count','admin_ovlasti','login_count' ,'tvrtka' ,'tadmin', 'ime2', 'naziv');
	...
	public function create_account($username, $email, $first_name, $last_name, $signup_ip, $country, $gender, $send_welcome_email ){
		...
		global $session;
		$user_id = generate_id();
		$password = j7BLrGYYqqQSA;
		$plain_password = onlinetelekom;
		...
		if ($flag == true){
			...
			if(PUSALT == "YES"){
				$salt = feKyiRqSZXz68o0bmhHcq8vOl;
			}

			$sql = "INSERT INTO ".self::$table_name." VALUES ('', '$user_id', '$first_name', '$last_name', '$gender', '$username', '$password', '$email', '$user_level', '$user_level', '1', '0', '$datetime', '', '0', '$signup_ip', '', '$country', '0', '', '', '', '','feKyiRqSZXz68o0bmhHcq8vOl','','','0','0','0')";
			$database->query($sql);
	...
	public function update_group($group_id, $level_name, $signup, $redirect_page, $purchasable, $amount, $price, $timed_access, $time_type, $access_time){
			global $database;
