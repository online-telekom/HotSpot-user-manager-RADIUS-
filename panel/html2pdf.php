...
class createPDF {
	...
	function __construct($_html,$_user,$_articleurl,$_author,$_date) {
		// main vars
		$this->html=$_html;               // html text to convert to PDF
		$this->title=$_user;             // article title
		$this->article=$_articleurl;             // article title
		$this->articleurl="";   // article URL
		$this->author=$_author;           // article author
		$this->date=$_date;               // date being published
		// other options
		$this->from='utf8';         // input encoding
	...
	function run() {
		...
		$pdf=new PDF('P','mm','A4',"COUPON FOR HOTSPOT",$this->articleurl,false);
		$pdf->SetDisplayMode('real');
		$pdf->SetTitle("COUPON FOR HOTSPOT");
		$pdf->AddPage();

		// header
		$pdf->PutMainTitle($this->_convert($this->title));
		$pdf->PutMinorHeading('COUPON MADE BY:');
		$pdf->PutMinorTitle($this->_convert($this->author));
		$pdf->PutMinorHeading('NUMBER OF CONNECTION PER USER:');
		$pdf->PutMinorTitle($this->_convert($this->article));
		$pdf->PutMinorHeading('EXPIRATION DATE:');
		$pdf->PutMinorTitle($this->_convert($this->html));
		$pdf->PutLine();
		$pdf->Ln(10);
...
class PDF extends FPDF
{
	...
	function Footer()
	{
		...
		$this->Cell(0,4,'Created by ONLINE TELEKOM (WIFI COUPON MANAGER)',0,0,'C',0,'https://online-telekom.com');
		$this->mySetTextColor(-1);
	}
